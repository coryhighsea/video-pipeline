...
export async function claudeEditFullVideo(params: {
  jobId: string;
  absInputPath: string;
  durationMs: number;
  captions: Caption[];
  ...
  console.log(`[claudeEdit:longform-${jobId.slice(0, 8)}] Sending ${captions.length} words to Claude for full-video edit...`);
...
Return the intervals to remove.`,
  ...
  console.log(`[claudeEdit:longform-${jobId.slice(0, 8)}] Assessment: ${object.clipAssessment}`);
  console.log(`[claudeEdit:longform-${jobId.slice(0, 8)}] ${removedIntervals.length} intervals flagged`);
  ...
  if (keepWindows.length === 0) {
    throw new Error("Claude full-video edit produced no keep windows");
  }
...
export async function claudeEditClip(params: {
  clipId: string;
  absInputPath: string;
  ...
  console.log(`[claudeEdit:${clipId.slice(0, 8)}] Sending ${captions.length} words to Claude for edit decisions...`);
...
Return only the intervals that genuinely need cutting. When in doubt, leave it in.`,
  ...
  const removedIntervals = mergeIntervals(validRemovals);
  console.log(`[claudeEdit:${clipId.slice(0, 8)}] Assessment: ${object.clipAssessment}`);
  console.log(`[claudeEdit:${clipId.slice(0, 8)}] Claude flagged ${removedIntervals.length} intervals to remove`);
  ...
  if (overCut) {
    console.warn(`[claudeEdit:${clipId.slice(0, 8)}] Claude would remove ${((1 - keepMs / totalSegmentMs) * 100).toFixed(0)}% of clip — skipping edit pass, using raw segments`);
  ...
  console.log(`[claudeEdit:${clipId.slice(0, 8)}] Keeping ${(keepMs / 1000).toFixed(1)}s of ${(totalSegmentMs / 1000).toFixed(1)}s total`);
